...
export async function sendGuestTranscriptEmail(
  ...
  const subject = sanitizeEmailHeaderValue(`Your ${BRAND_SPOKEN} help chat`);
  const text = `${greeting}
...
<p>— ${escapeHtml(BRAND_SPOKEN)}</p>`;
  ...
  // Mark after successful send so a crash mid-send can retry on the next cron.
